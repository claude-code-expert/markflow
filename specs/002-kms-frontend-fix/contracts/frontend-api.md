...
Response 201:
  Body: {
    user: { id: number, email, name, avatarUrl, emailVerified, createdAt, updatedAt }
  }
...
Response 201:
  Body: {
    workspace: { id, name, isRoot, isPublic, ownerId, createdAt, updatedAt }
  }

Error 409: { error: { code: "NAME_EXISTS", message: string } }
```
...
Response 200:
  Body: {
    workspace: { id, name, isRoot, isPublic, ownerId, createdAt, updatedAt, memberCount: number }
  }
```
...
```
Request:
  Body: { title: string, categoryId?: number, content?: string }
...
**Frontend action**: 생성 후 `/${encodeURIComponent(workspaceName)}/doc/${document.id}` 에디터로 이동
...
```
Response 200:
  Body: { tags: [{ id: number, workspaceId: number, name: string }] }
```

...
> **변경 이력:**
> - v1.3.0 — Phase 1 구현 완료 반영, 기능별 구현 상태(✅/📋) 표시, 댓글·임베드·테마·프레젠테이션 기능 추가, OAuth Phase 3 이연 명시
...
| **KMS** | `apps/` | 팀 지식 관리 SaaS (인증, 워크스페이스, 협업 등) | ✅ Phase 1 구현 완료 (2026-04-04) |
...
## Part B. KMS SaaS (✅ Phase 1 구현 완료 — 2026-04-04)

> Phase 1 기능이 구현 완료되었으며, 각 항목에 구현 상태를 표시한다.
...
| 소셜 로그인 (Google, GitHub) | P1 → **Phase 3** | OAuth 콜백, 계정 연결/해제 | 📋 Phase 3 이연 |
| 프로필 편집 | P0 | 이름, 아바타(JPG/PNG, 2MB), 비밀번호 변경 | ✅ |
| 로그아웃 | P0 | Refresh Token 즉시 무효화, 전체 기기 로그아웃 | ✅ |

**주요 인수 조건**
- [x] 비밀번호: 8자 이상, 영문+숫자+특수문자 ✅
- [x] 동일 계정 5회 연속 로그인 실패 시 15분 **계정 잠금** → `401 + { code: "ACCOUNT_LOCKED" }` 반환 ✅
- [x] IP 기준 10회/15분 초과 시 **Rate Limit** → `429 + { code: "RATE_LIMITED" }` 반환 (계정 잠금과 별개 동작) ✅
- [x] Remember me 선택 시 Refresh Token 30일 ✅
...
| 기능 | 우선순위 | 설명 | 상태 |
|------|---------|------|------|
| 워크스페이스 생성 | P0 | name 기반 URL (UNIQUE). 플랜별 제한: Free=1개(Root 포함), Team=무제한 | ✅ |
| **워크스페이스 리스트 뷰** | P0 | 카드 그리드 아님. 행 기반 리스트. 역할 배지·활동 시간 표시 | ✅ |
| 멤버 초대 | P0 | 이메일 초대, 72시간 유효 | ✅ |
| 역할 관리 | P0 | Owner / Admin / Editor / Viewer | ✅ |
| **가입 요청 (Join Request)** | P0 | 공개 워크스페이스에 비멤버가 신청. 신청 메시지 포함 | ✅ |
| **가입 요청 관리** | P0 | Admin/Owner: 신청자 검토 → 역할 지정 후 승인/거절. 일괄 처리 | ✅ |
...
**가입 요청 플로우** ✅
1. 비멤버가 워크스페이스 검색 → `가입 요청` 클릭
2. 신청 메시지 입력 (선택) → `POST /workspaces/:id/join-requests`
3. 워크스페이스 설정 > 멤버 관리 > **가입 요청 탭** 에 `대기` 상태로 노출
...
| 가입 요청 | 신청자 메시지 · 역할 선택 · 승인/거절 · 일괄 처리 |
...
| 항목 | 동작 | 상태 |
|------|------|------|
| 새 문서 추가 | 해당 폴더를 기본값으로 새 문서 모달 오픈 | ✅ |
| 하위 폴더 추가 | 하위 경로 미리보기 포함 새 폴더 모달 오픈 | ✅ |
| 이름 변경 | 폴더 이름 변경 모달 → 사이드바 DOM 즉시 반영 | ✅ |
| 위치 이동 | 드래그 앤 드롭 | 📋 Phase 2 |
| 폴더 삭제 | 이름 입력 확인 → 문서는 루트 이동, 폴더 제거 | ✅ |
...
- [x] 삭제된 폴더의 문서는 루트로 이동 ✅
- [x] 무제한 중첩 폴더 구조 (백엔드 Closure Table) ✅
- [ ] 드래그-드롭 이동 📋 Phase 2
- [x] 접기/펼치기 상태 localStorage 유지 ✅
...
- [x] Admin 이상 워크스페이스 단위 CSS 편집 (CodeMirror CSS 모드) ✅
- [x] CSS 변수 기반 (`--mf-font-body`, `--mf-color-heading` 등) ✅
- [x] 프리셋: Default, GitHub, Notion, Dark, Academic (5종) ✅
- [x] CSS 오버라이드 지원 ✅
...
- [x] 전문 검색 (제목 > 첫 단락 > 본문 가중치) ✅
- [x] `Ctrl+/` 단축키 전역 검색 팝업 ✅
- [x] 카테고리/태그/작성자/날짜 범위 필터 ✅
...
| 기능 | 설명 | 상태 |
|------|------|------|
| 실시간 공동 편집 | Yjs CRDT 기반, 최대 30인, 원격 커서 표시 | 📋 Phase 2 |
| 댓글 시스템 | 댓글 생성, 삭제, 중첩 스레드 | ✅ |
| 활동 피드 | 워크스페이스 이력, @멘션 알림 | 📋 Phase 2 |
...
| 댓글 생성 | 문서에 댓글 작성 | ✅ |
| 댓글 삭제 | 본인 댓글만 삭제 가능 | ✅ |
| 중첩 스레드 | 댓글에 대한 답글 (스레드형) | ✅ |

---

### B11. Root 워크스페이스 `P0` ✅

- [x] 회원가입(이메일 인증) 완료 시 서버가 **Root 워크스페이스 1개를 자동 생성** (이름: "My Notes") ✅
- [x] Root 워크스페이스는 삭제 불가 (Owner 본인만 소속) ✅
- [x] 이름 변경 가능, CSS 테마 적용 가능 ✅
- [x] 문서 생성 시 워크스페이스 미지정 → Root 워크스페이스에 자동 귀속 ✅
...
| 기능 | 설명 | 상태 |
|------|------|------|
| 토큰 생성 | 라벨 + 범위(scope) + 만료일 설정 | ✅ |
| 토큰 조회 | 발급된 토큰 목록 확인 | ✅ |
| 토큰 폐기 | 발급된 토큰 즉시 무효화 | ✅ |

**인수 조건**
- [x] Guest Token: 만료일 설정, 읽기 전용/읽기-쓰기 scope 선택 ✅
- [x] iframe 모드: `?readOnly=true`, `?theme=dark`, `?hideToolbar=true` 쿼리 파라미터 지원 ✅
- [x] NPM 패키지: KMS 연결 없이도 동작 (localStorage 저장 폴백) ✅
...
| 기능 | 설명 | 상태 |
|------|------|------|
| 프레젠테이션 진입 | 문서 에디터에서 프레젠테이션 버튼으로 전체화면 진입 | ✅ |
| 어노테이션 도구 | 펜, 지우개, 색상 선택 | ✅ |
| TOC 내비게이션 | 목차 기반 섹션 이동 | ✅ |
| 폰트 크기 조절 | 발표 상황에 맞게 글자 크기 변경 | ✅ |
| 종료 | ESC 키 또는 종료 버튼으로 프레젠테이션 종료 | ✅ |
...
| 기능 | 설명 | 상태 |
|------|------|------|
| 가입 요청 제출 | 공개 워크스페이스에 메시지와 함께 가입 요청 | ✅ |
| 요청 승인/거절 | Admin/Owner가 요청을 검토 후 승인 또는 거절 | ✅ |
| 일괄 처리 | 다수 요청을 한번에 승인/거절 | ✅ |
...
| 기능 | 설명 | 상태 |
|------|------|------|
| 소프트 삭제 | 문서 삭제 시 휴지통으로 이동 (즉시 영구 삭제 아님) | ✅ |
| 복원 | 휴지통에서 문서를 원래 위치로 복원 | ✅ |
| 영구 삭제 | 휴지통에서 수동 영구 삭제 가능 | ✅ |
| 자동 정리 | 30일 경과 시 자동 영구 삭제 | ✅ |

...
> **상태:** 구현 진행 중 (엔드포인트별 구현 상태 표시)
> **변경 이력:**
> - v1.3.0 — 실제 구현 기준 동기화: 엔드포인트별 구현 상태(✅/⏳/📋) 표시, `/links`→`/relations` 경로 반영, `/categories/tree` 추가, `/join-requests/batch` 추가, Theme·Embed Token·Graph API 섹션 추가, Comments API PATCH 미구현 명시, startMode 미구현 명시
> - v1.2.0 — Section 4 Categories API 상세화 (이동·Closure Table 동기화·에러 코드), Section 5 POST documents `startMode` 파라미터 추가, Section 13 그래프 뷰 API 신규 추가
...
**Response** `201` `{ "message": "인증 이메일이 발송되었습니다.", "userId": 1 }`
...
### GET /workspaces ✅
내가 속한 워크스페이스 목록
**Response** `200` `{ workspaces: [{ id, name, role, memberCount, documentCount, updatedAt }] }`

### POST /workspaces ✅
**Request** `{ "name": "...", "description": "..." }`
**Response** `201` `{ id, name, role: "owner" }`

### GET /workspaces/:workspaceId ✅
**Response** `200` `{ id, name, description, themeCss, isPublic, memberCount, createdAt }`
...
**Response** `200` `{ workspaceId, workspaceName, role }`
...
공개 워크스페이스에 가입 요청
**Request** `{ "message": "..." }`
**Response** `201` `{ requestId, status: "pending" }`

### PATCH /workspaces/:workspaceId/join-requests/:requestId `[Admin+]` ✅
가입 요청 승인/거절
**Request** `{ "status": "approved" | "rejected" }`
**Response** `200`

### PATCH /workspaces/:workspaceId/join-requests/batch `[Admin+]` ✅
가입 요청 일괄 승인/거절
**Request** `{ "requestIds": [1, 2, 3], "status": "approved" | "rejected" }`
**Response** `200` `{ processed: 3, results: [{ requestId, status, success }] }`

---

## 3-1. 워크스페이스 테마 (Theme API)

### GET /workspaces/:workspaceId/theme ✅
워크스페이스 테마 설정 조회
**Response** `200` `{ themePreset, themeCss }`

### PATCH /workspaces/:workspaceId/theme `[Admin+]` ✅
**Request** `{ "themePreset": "default" | "ocean" | "forest" | "...", "themeCss": "..." }`
**Response** `200` `{ themePreset, themeCss }`
...
### GET /workspaces/:workspaceId/categories/tree ✅
카테고리 트리 전체 (중첩 `children` + 소속 문서 포함, Closure Table 기반)
**Response** `200`
```json
{
  "categories": [
    {
      ...
      "documentCount": 12,
      "children": [
        { "id": 2, "name": "Sub Category", "parentId": 1, "depth": 1, "children": [] }
      ],
      "documents": [
        { "id": 10, "title": "기능 요구사항", "slug": "기능-요구사항", "updatedAt": "..." }
      ]
    }
  ]
}
...
> **참고:** 카테고리와 문서는 여전히 `slug` 필드를 유지한다. 워크스페이스에서만 `slug`가 제거되었다.
...
**Request** `{ "newParentId": "number | null" }`
...
**Query:** `categoryId`, `sort` (updatedAt|createdAt|title), `order` (asc|desc), `page`, `limit`, `deleted`, `q` (기본 검색)
**Response** `200` `{ documents: [{ id, title, slug, categoryId, authorName, currentVersion, updatedAt }], total, page, limit }`

### POST /workspaces/:workspaceId/doc `[Editor+]` ✅
**Request**
```json
{
  "title": "문서 제목",
...
```
> **참고:** `startMode` 파라미터는 📋 미구현 상태. 현재 모든 문서는 `blank` 모드로 생성됨.
...
```json
{
  ...
  "content": "...",
  "categoryId": 5,
  "categoryPath": [{ "id": 5, "name": "001 Requirements" }],
  "authorId": 1,
  "currentVersion": 12,
  "tags": ["requirements", "p0"],
  "dagLinks": {
    "prevDoc":     { "id": 2, "title": "000 — 프로젝트 개요" },
    "nextDoc":     { "id": 3, "title": "002 — 컴포넌트 설계" },
    "relatedDocs": [
      { "id": 7, "title": "007 — 아키텍처" },
      { "id": 8, "title": "008 — 로드맵" }
    ]
...
자동저장 포함
**Request** `{ "title": "...", "content": "...", "categoryId": 5 }`
...
### 문서 관계 (Document Relations)

> **v1.3.0 변경:** 경로가 `/links`에서 `/relations`으로 변경됨.

### PUT /workspaces/:workspaceId/documents/:documentId/relations `[Editor+]` ✅
**Request** `{ "prevDocId": "number|null", "nextDocId": "number|null", "relatedDocIds": [7] }`
> 연관 문서(`relatedDocIds`) 최대 20개 초과 시 `400 { code: "TOO_MANY_RELATED_DOCS" }` 반환

### GET /workspaces/:workspaceId/documents/:documentId/relations ✅
**Response** `200`
```json
{
  "prevDoc": { "id": 2, "title": "000 — 프로젝트 개요" },
...
> **현재 상태:** 전용 검색 API는 미구현. 현재 문서 목록 API(`GET /workspaces/:wsId/documents`)의 `q` 파라미터로 기본 검색만 가능.
...
> **현재 상태:** 댓글 수정 및 resolved 처리는 📋 미구현.
...
```json
{
  ...
  "allowedDocIds": [1],               // 특정 문서만 허용 (null = 워크스페이스 전체)
  ...
  "rows": [
    {
      ...
      "stages": [
        ...
        {
          "type": "category",
          "nodes": [{ "id": 5, "title": "001 Requirements", "documentCount": 12 }]
        },
        {
          "type": "prev",
          "nodes": [{ "id": 2, "title": "000 — 프로젝트 개요", "updatedAt": "..." }]
        },
        {
          "type": "current_group",
          "label": "현재 문서 + 연관",
          "nodes": [
            { "id": 1, "title": "001 — 기능 요구사항", "nodeType": "current" },
            { "id": 7, "title": "007 — 아키텍처",     "nodeType": "related", "tagLink": "architecture" },
            { "id": 8, "title": "008 — 로드맵",       "nodeType": "related", "tagLink": "roadmap" }
          ]
        },
        {
          "type": "next",
          "nodes": [{ "id": 3, "title": "002 — 컴포넌트 설계", "updatedAt": "..." }]
        }
      ]
    },
    {
      ...
      "stages": [
        {
          ...
          "tag": "requirements",
          "nodes": [
            { "id": 1, "title": "001 — 기능 요구사항" },
            { "id": 2, "title": "000 — 프로젝트 개요" }
          ]
        }
      ]
    }
...
| 파라미터 | 타입 | 기본값 | 설명 |
|---------|------|--------|------|
| `focusDocId` | `number` | — | 특정 문서 중심으로 필터링 |
| `categoryId` | `number` | — | 특정 카테고리 Row만 반환 |
...
```json
{
  "document": { "id": 1, "title": "001 — 기능 요구사항" },
  "category": { "id": 5, "name": "001 Requirements" },
  "prev":    { "id": 2, "title": "000 — 프로젝트 개요" },
  "next":    { "id": 3, "title": "002 — 컴포넌트 설계" },
  "related": [
    { "id": 7, "title": "007 — 아키텍처", "sharedTags": ["architecture"] },
    { "id": 8, "title": "008 — 로드맵",   "sharedTags": ["roadmap"] }
  ]
}
```
...
아래는 현재 📋 미구현 상태인 엔드포인트를 우선순위별로 정리한 목록이다.

### P1 — 핵심 기능 (다음 스프린트 구현 대상)

| 엔드포인트 | 설명 |
|-----------|------|
| `POST /auth/forgot-password` | 비밀번호 분실 이메일 발송 |
| `POST /auth/reset-password` | 비밀번호 재설정 |
| `PATCH /users/me/password` | 비밀번호 변경 |
| `PATCH .../comments/:commentId` | 댓글 수정 및 resolved 처리 |
| `GET .../documents/:docId/versions` | 문서 버전 이력 조회 |
| `GET .../documents/:docId/versions/:versionNum` | 특정 버전 내용 조회 |
| `POST .../documents/:docId/restore-version` | 특정 버전으로 복원 |
| `GET .../documents/:docId/versions/diff` | 버전 간 diff |
| `GET /workspaces/:wsId/search` | 전용 검색 API (현재 `q` 파라미터 기본 검색만 가능) |

### P2 — 확장 기능

| 엔드포인트 | 설명 |
|-----------|------|
| `POST /link-preview` | OG 메타데이터 기반 링크 프리뷰 |
| `POST .../import` | 문서 Import (md/zip/html) |
| `GET .../export` | 문서 Export (md/zip/html/pdf) |
| `GET /embed/doc/:documentId` | iframe embed용 문서 뷰어 |
| `GET .../documents/:docId/dag-context` | 문서별 미니 DAG 데이터 |

### P3 — 부분 구현 (완성 필요)

| 엔드포인트 | 상태 | 설명 |
|-----------|------|------|
| `PUT /users/me/avatar` | ⏳ | R2 업로드 미구현, 플레이스홀더 URL만 반환 |
| `POST .../doc` — `startMode` 파라미터 | 📋 | 템플릿 기반 문서 생성 미구현 (항상 blank) |

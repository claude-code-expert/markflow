...
> **변경 이력:**
> - v1.3.0 — 통합 테스트 26개 파일 현황 반영, 팩토리 함수(createUser/createWorkspace/addMember/createInvitation/createJoinRequest) 문서화, 프레젠테이션·임베드·댓글 시나리오 추가
> - v1.2.0 — 2.4 폴더 관리 비즈니스 로직 테스트, 3.4 Categories API 테스트, 4.4 폴더 관리 E2E, 4.5 그래프 뷰 E2E 추가
...
| 레이어 | 상태 | 비고 |
|--------|------|------|
| Unit | ✅ 완료 | 에디터 유틸리티, 컴포넌트, 비즈니스 로직 |
| Integration | ✅ 완료 (26 files) | `apps/api/tests/` 기준 |
| E2E | 📋 계획됨 | Playwright 시나리오 정의 완료, 구현 예정 |
...
### 3.0 통합 테스트 파일 인벤토리 (26 files)

**위치:** `apps/api/tests/`

| # | 파일 | 영역 |
|---|------|------|
| 1 | `integration/auth-login.test.ts` | 로그인, 계정 잠금, Rate Limit |
| 2 | `integration/auth-refresh.test.ts` | 토큰 갱신 |
| 3 | `integration/auth-register.test.ts` | 회원가입 |
| 4 | `integration/auth-verify.test.ts` | 이메일 인증 |
| 5 | `integration/categories.test.ts` | 카테고리 CRUD, 계층 구조 |
| 6 | `integration/circular-ref.test.ts` | 순환 참조 감지 |
| 7 | `integration/data-isolation.test.ts` | 워크스페이스 간 데이터 격리 |
| 8 | `integration/documents.test.ts` | 문서 CRUD, 버전, 태그 |
| 9 | `integration/embed-tokens.test.ts` | 임베드 토큰 발급·폐기·만료 |
| 10 | `integration/export.test.ts` | HTML/PDF Export |
| 11 | `integration/graph.test.ts` | 관계 그래프 조회 |
| 12 | `integration/import.test.ts` | 문서 Import |
| 13 | `integration/invitations.test.ts` | 초대 생성·수락·만료 |
| 14 | `integration/join-requests.test.ts` | 가입 요청 |
| 15 | `integration/kms-full-flow.test.ts` | KMS 전체 플로우 시나리오 |
| 16 | `integration/members.test.ts` | 멤버 관리 |
| 17 | `integration/public-workspaces.test.ts` | 공개 워크스페이스 |
| 18 | `integration/rbac-matrix.test.ts` | 역할 기반 접근 제어 매트릭스 |
| 19 | `integration/relations.test.ts` | 문서 간 연관 관계 |
| 20 | `integration/security-audit.test.ts` | 보안 감사 |
| 21 | `integration/tags.test.ts` | 태그 CRUD |
| 22 | `integration/theme.test.ts` | 테마 프리셋·CSS 오버라이드 |
| 23 | `integration/trash.test.ts` | 휴지통·복원·영구 삭제 |
| 24 | `integration/users-me.test.ts` | 사용자 프로필 |
| 25 | `integration/workspaces.test.ts` | 워크스페이스 CRUD |
| 26 | `unit/slug.test.ts` | Slug 생성 유틸리티 |

**추가 테스트 (부하·검증):**

| 파일 | 영역 |
|------|------|
| `load/concurrent-users.test.ts` | 동시 사용자 부하 |
| `load/document-list.test.ts` | 문서 목록 부하 |
| `unit/theme-validator.test.ts` | 테마 CSS 검증 |
...
describe('Comments API', () => {
  it('Editor 이상 멤버가 문서에 댓글을 작성한다 → 201')
  it('Viewer 멤버는 댓글 작성 시 403 반환')
  it('댓글 작성자 본인만 삭제할 수 있다 → 204')
  it('다른 사용자의 댓글 삭제 시도 시 403 반환')
  it('중첩 스레드: 댓글에 답글 작성 시 parentId가 설정된다')
  it('중첩 스레드: 부모 댓글 삭제 시 하위 답글도 soft-delete 처리')
  it('Admin 이상은 다른 사용자의 댓글을 삭제할 수 있다')
})
```
...
describe('Embed Tokens API', () => {
  it('Admin 이상이 POST /embed-tokens로 토큰을 생성한다 → 201')
  it('생성된 토큰 목록을 GET /embed-tokens로 조회한다 → 200')
  it('DELETE /embed-tokens/:id로 토큰을 폐기한다 → 204')
  it('폐기된 토큰으로 embed 접근 시 401 반환')
  it('만료 시각이 지난 토큰으로 접근 시 401 + TOKEN_EXPIRED 반환')
  it('유효한 토큰으로 embed 접근 시 200 + 문서 내용 반환')
})
```
...
```typescript
describe('Theme API', () => {
  it('프리셋 테마를 워크스페이스에 적용한다 → 200')
  it('CSS 변수 오버라이드를 저장한다 → 200')
  it('Viewer 역할의 테마 변경 시 403 반환')
  it('유효하지 않은 CSS 속성 제출 시 400 반환')
})
```
...
```typescript
describe('Graph API', () => {
  it('문서 간 관계 그래프를 조회한다 → 200 + nodes/edges 반환')
  it('특정 문서의 이웃 노드 통계를 반환한다')
  it('워크스페이스 범위 외 문서 접근 시 403 반환')
})
```
...
### 4.6 프레젠테이션 모드 E2E (신규)

```typescript
test('프레젠테이션 모드에 진입하고 어노테이션 도구를 사용한다', async ({ page }) => {
  // 1. 문서 에디터에서 프레젠테이션 모드 진입 버튼 클릭
  // 2. 전체 화면 프레젠테이션 UI 표시 확인
  // 3. 어노테이션 도구 (펜, 형광펜, 포인터) 선택 및 사용
  // 4. 어노테이션 그리기 → 캔버스에 렌더링 확인
})

test('프레젠테이션 모드에서 TOC 네비게이션으로 섹션을 이동한다', async ({ page }) => {
  // 1. 프레젠테이션 모드 진입
  // 2. TOC 패널 열기 → 헤딩 목록 표시 확인
  // 3. 특정 헤딩 클릭 → 해당 섹션으로 스크롤 이동
  // 4. 현재 섹션 인디케이터 업데이트 확인
})

test('프레젠테이션 모드를 종료하면 에디터로 복귀한다', async ({ page }) => {
  // 1. 프레젠테이션 모드 진입
  // 2. ESC 키 또는 종료 버튼 클릭
  // 3. 에디터 화면 복귀 확인
  // 4. 어노테이션 데이터가 정리되었는지 확인
})
```
...
**위치:** `apps/api/tests/helpers/factory.ts`

통합 테스트에서 재사용하는 팩토리 함수 모음. 모든 함수는 Drizzle ORM `Db` 인스턴스를 첫 번째 인자로 받으며, 실제 DB에 레코드를 삽입한 뒤 결과를 반환한다.

| 함수 | 시그니처 | 기본값 | 설명 |
|------|----------|--------|------|
| `createUser` | `(db, options?)` | `user{N}@test.com`, `Test123!@#`, `emailVerified: true` | 테스트 사용자 생성. Access Token도 함께 반환 |
| `createWorkspace` | `(db, ownerId, options?)` | `Workspace {N}`, `workspace-{N}`, `isPublic: true` | 테스트 워크스페이스 생성. Owner 멤버 자동 추가 |
| `addMember` | `(db, workspaceId, userId, role)` | — | 워크스페이스에 멤버 추가 (`admin` / `editor` / `viewer`) |
| `createInvitation` | `(db, workspaceId, inviterId, options?)` | `editor` 역할, 7일 만료, `pending` 상태 | 워크스페이스 초대 생성. 랜덤 토큰 자동 발급 |
| `createJoinRequest` | `(db, workspaceId, userId, options?)` | `pending` 상태, 기본 메시지 | 워크스페이스 가입 요청 생성 |

```typescript
// 사용 예시
const { user: owner, accessToken } = await createUser(db);
const workspace = await createWorkspace(db, owner.id);
const { user: viewer } = await createUser(db, { email: 'viewer@test.com' });
await addMember(db, workspace.id, viewer.id, 'viewer');
const invitation = await createInvitation(db, workspace.id, owner.id, { role: 'editor' });
const joinReq = await createJoinRequest(db, workspace.id, viewer.id);
```
